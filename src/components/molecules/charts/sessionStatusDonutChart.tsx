	...
	if (isLoading) {
		return (
			<div className={className}>
				<div className="flex h-full animate-pulse flex-row items-center justify-center gap-6">
			...
			<div className="flex h-full flex-row items-center justify-center gap-6">
				<div className="relative w-24 shrink-0 sm:w-40">
					<DonutChart
						...
						category="value"
						className="[&_text]:hidden sm:[&_text]:block"
						colors={colors}
						data={chartData}
						index="name"
						showAnimation
					...
					/>
					<div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center sm:hidden">
						<span className="font-fira-code text-sm font-medium tabular-nums text-white">
							{total.toLocaleString()}
						</span>
						<span className="font-fira-code text-10 tabular-nums text-gray-400">100%</span>
					</div>
				</div>
				{showLegend ? (
					<div className="flex flex-col gap-3">
