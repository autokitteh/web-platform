...
interface OrgConnectionInfoPopoverProps {
	connectionId: string;
...
export const OrgConnectionInfoPopover = ({ connectionId, icon }: OrgConnectionInfoPopoverProps) => {
	const { t } = useTranslation("tabs", { keyPrefix: "connections.table.popover" });
	const { t: tIntegrations } = useTranslation("integrations");
	const connection = useGlobalConnectionsStore((state) =>
	...
	if (!icon) {
		LoggerService.warn("OrgConnectionInfoPopover", `Missing icon for connection: ${connectionId}`, true);
			...
			<PopoverContent className="z-40 rounded-lg border-0.5 border-white bg-black p-4">
				<div className="text-white">
					...
					<dl className="flex items-center gap-x-1">
						...
						<dd data-testid="org-connection-detail-connection-id">
					...
					{authType ? (
						<dl className="flex items-center gap-x-1">
							...
							<dd data-testid="org-connection-detail-auth-type">
