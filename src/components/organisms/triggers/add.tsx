...
export const AddTrigger = () => {
	...
	const formId =
		selectedType?.value === TriggerFormType.default
			? TriggerFormIds.createNewDefaultForm
			: TriggerFormIds.createNewSchedulerForm;

	return (
		<div className="min-w-80">
			<TabFormHeader className="mb-11" form={formId} isLoading={isLoading} title={t("addNewTrigger")} />
			<div className="flex flex-col gap-6">
				...
				/>
				{selectedType?.value === TriggerFormType.default ? (
					<DefaultTriggerForm formId={formId} setIsSaving={setIsLoading} />
				) : null}
				{selectedType?.value === TriggerFormType.scheduler ? (
					<TriggerSchedulerForm formId={formId} setIsSaving={setIsLoading} />
				) : null}
			</div>
		</div>
	);
