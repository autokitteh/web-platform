...
export class SessionLogRecord {
	...
	constructor(logRecord: ProtoSessionLogRecord) {
		...
		switch (logRecordType) {
			...
			case SessionLogRecordType.print:
				...
				this.logs = `${i18n.t("sessions.historyPrint")}: ${logRecord.print!.text}`;
				break;
		}
	...
	private handleStateRecord(logRecord: ProtoSessionLogRecord) {
		...
		if (this.state === SessionStateType.running) {
			...
			this.logs = functionRunning ? `${i18n.t("sessions.historyInitFunction")}: ${functionRunning}` : undefined;
		}
	...
	private handleCallAttemptComplete(logRecord: ProtoSessionLogRecord) {
		...
		if (logRecord[this.type]?.result?.value?.time) {
			this.logs =
				...
				`${i18n.t("sessions.historyFunction")} - ${i18n.t("sessions.historyResult")}: ${i18n.t("sessions.historyTime")} - ${convertTimestampToDate(logRecord[this.type]?.result?.value?.time?.v).toISOString()}`;
		...
		if (logRecord[this.type]?.result?.value?.nothing) {
			this.logs =
				// eslint-disable-next-line max-len
				`${i18n.t("sessions.historyFunction")} - ${i18n.t("sessions.historyResult")}: ${i18n.t("sessions.historyNoOutput")}`;
			return;
		}
		...
		this.logs =
			// eslint-disable-next-line max-len
			`${i18n.t("sessions.historyFunction")} - ${i18n.t("sessions.historyResult")}: ${functionName} - ${functionResponse}`;
	}
