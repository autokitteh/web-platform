...
export const processToursFromTemplates = async (structure?: Record<string, any>) => {
	...
	const processDirectory = async (dirStructure: Record<string, any>, currentPath: string = "") => {
		for (const [name, node] of Object.entries(dirStructure)) {
			...
			if (currentPath !== toursDir && !currentPath.startsWith(`${toursDir}/`)) {
				await processDirectory(node.children, fullPath);
				continue;
			}
